...
class PosOrder(models.Model):
    ...
    def action_generate_sunat_xml(self):
        for order in self:
            ...
            serie = (
                config.sunat_serie_factura
                if tipo == "01"
                else config.sunat_serie_boleta
            )
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
    ...
    <cac:LegalMonetaryTotal>
        <cbc:PayableAmount currencyID="PEN">{order.amount_total:.2f}</cbc:PayableAmount>
    </cac:LegalMonetaryTotal>
</Invoice>
"""
            ...
            extension_content = signed_root.find(
                ".//ext:ExtensionContent", namespaces=ns
            )
    ...
    def action_send_sunat(self):
        for order in self:
            ...
            try:
                if not order.sunat_zip_file or not order.sunat_zip_filename:
        ...
        <ser:sendBill>
            ...
            <contentFile>{zip_base64}</contentFile>
                ...
                order.write(
                    {
                        "sunat_state": "error",
                        "sunat_message": f"Error envío SUNAT manual SOAP: {str(e)}",
                    }
                )
